...
void SrVMachine::processing(int timeOffset)
  {
  //Инкремент счетчика времени
  memSet( 0, 1, memGet(0,1) + timeOffset );
...
bool SrVMachine::executeMethod(SrVmVpu *vpu, int methodId)
  {
  switch( methodId ) {
    case VPS_CREATETASK :
    ...
    case VPS_UMIN :
      ...
      VPU_SET_FUN_RESULT( 2, svUMin( VPU_GET_FUN_PARAM(1), VPU_GET_FUN_PARAM(0) ) );
      break;

    case VPS_UMAX :
      //2              1       0
      //int  UMax( int v1, int v2 );
      VPU_SET_FUN_RESULT( 2, svUMax( VPU_GET_FUN_PARAM(1), VPU_GET_FUN_PARAM(0) ) );
      break;
    ...
    case VPS_FACTOR106 :
      ...
      VPU_SET_FUN_RESULT( 2, (int)( ((double)VPU_GET_FUN_PARAM(1)) * (((double)VPU_GET_FUN_PARAM(0)) / 1000.0) / 1000.0 ) );
      break;

    case VPS_FACTOR103 :
      //2                   1          0
      //int  Factor103( int val, int factor );
      VPU_SET_FUN_RESULT( 2, (int)( ((double)VPU_GET_FUN_PARAM(1)) * (((double)VPU_GET_FUN_PARAM(0)) / 1000.0) ) );
      break;

    case VPS_DEFACTOR106 :
      //2                   1          0
      //int  DeFactor106( int val, int factor );
      VPU_SET_FUN_RESULT( 2, (int)( ((double)VPU_GET_FUN_PARAM(1)) * 1000.0 / (((double)VPU_GET_FUN_PARAM(0)) / 1000.0) ) );
      break;
...
void SrVMachine::setProgramm( const SrVmCode *code )
  {
  mVpuCount = 0;
  mProg = code;
  mProgSize = mProg == 0 ? 0 : progSize();
  }
...
void SrVMachine::reset()
  ...
  //Подготовить память
  if( mMemorySize )
    memset( mMemory, 0, mMemorySize * sizeof(int) );
  }
...
void SrVMachine::restart(bool run)
  {
  reset();

  if( mMemory == 0 || mProg == 0 || mVpuList == 0 )
    return;
  ...
  if( mProgSize > 0 ) {
    ...
    int table = initTable();
    for( int i = 0; value3( 0, table + i ) != 0; i += 7 )
      memSet( 0, value3( 0, table + i), value4( 0, table + i + 3) );
...
//Отладка - пуск
void SrVMachine::debugRun(int vpu)
  {
...
//Отладка - стоп (пауза)
void SrVMachine::debugPause(int vpu)
  {
...
void SrVMachine::debugRunTrace(int vpu, int start, int stop)
  {
  //Проверить границы VPU
  if( vpu < 0 || vpu >= mVpuCount ) return;
...
void SrVMachine::debugRunStep(int vpu, int start, int stop)
  {
  //Проверить границы VPU
  if( vpu < 0 || vpu >= mVpuCount ) return;
...
void SrVMachine::debugRunUntil(int vpu, int start, int stop)
  {
  //Проверить границы VPU
  if( vpu < 0 || vpu >= mVpuCount ) return;
...
void SrVMachine::executeCore(SrVmVpu *vpu)
  ...
  while( vpu->mIp ) {
    ...
    //Исполнение команд
    switch( static_cast<SrVmByteCode>(mProg[vpu->mIp]) )
      {
      //Команда останова
      case VBC1_HALT :
        vpu->mIp = 0;
...
void SrVMachine::executeHandler0( int addr ) {
...
void SrVMachine::executeHandler1( int addr, int value0 ) {
...
void SrVMachine::executeHandler2( int addr, int value0, int value1 ) {
  //Разместить параметр в стеке
  memSet( mVpuList, --(mVpuList[0].mSp), value0 );
...
void SrVMachine::callInternal( SrVmVpu *vpu, int addr, int retAddr ) {
  //внутренние функции
...
void SrVMachine::call(SrVmVpu *vpu, int addrOffset)
  {
  //Вызов любой функции
  ...
  if( tmp & 0x80000000 ) {
    ...
    if( executeMethod( vpu, tmp & 0xefffffff ) )
      return;
    vpu->mIp += 2;
    }
  else
