...
class SvMirror : public QObject
  ...
  signals:
    ...
    //!
    //! \brief ticket   Извещение для удаленного управления и удаленного вызова процедур.
    //! \param ticketId Идентификатор извещения
    //! \param px       Параметр извещения
    //!
    //! При взаимодействии хоста с удаленным контроллером существует проблема среза данных.
    //! Поскольку данные передаются последовательно, то возможна следующая ситуация:
    //! например скрипт сначала изменяет d0, затем d1, но, если
    //! это изменение происходит после того как d0 уже передано, а d1 еще нет, то на приемной стороне
    //! некоторое время d1 будет новое, а d0 еще предыдущее. Такая ситуация будет сохраняться до
    //! следующего цикла чтения. В приложениях взаимодействия для борьбы с этой ситуацией служит
    //! извещение. Данные через извещение передаются все вместе после того, как они все вместе
    //! установлены.
    //!
    void ticket( int ticketId, int p0, int p1, int p2, int p3, int p4, int p5, int p6, int p7 );
