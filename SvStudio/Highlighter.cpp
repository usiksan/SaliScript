...
int
Highlighter::MultiLineComment(const QString &text, int index, int count) {
...
void Highlighter::setLink(const QString link)
  {
  mLink = link;
  }
...
Highlighter::highlightBlock(const QString &text) {
  ...
  while( index < count ) {
    ...
    if( text[index] == QChar('#') ) {
      int startIndex = index++;
      while( index < count && text[index].isLetter() ) index++;
      setFormat( startIndex, index - startIndex, mPreprocessFormat );
      }
