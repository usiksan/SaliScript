...
SrCompiler::Reset() {
  ...
  mTypeList.addType( mTypeInt        = new SrType("int", CLASS_INT, 1, 0) );  //Целое
  mTypeList.addType( mTypeIntPtr     = new SrType("*int", CLASS_POINTER, 1, mTypeInt ) );
  mTypeList.addType( mTypeCString    = new SrType("cstring", CLASS_CSTRING, 1, 0 ) );
  mTypeList.addType( mTypeCStringPtr = new SrType("*cstring", CLASS_POINTER, 1, mTypeCString ) );
  mTypeList.addType( mTypeVoid       = new SrType("void", CLASS_VOID, 0, 0 ) );
  mTypeList.addType( mTypeVoidPtr    = new SrType("*void", CLASS_POINTER, 1, mTypeVoid ) );
  mTypeList.addType( mTypeFail       = new SrType("fail type", 0, 0, 0 ) );
  mTypeList.addType( mTypeCBlock     = new SrType("cblock", CLASS_CBLOCK, 0, 0 ) );
  mTypeList.addType( mTypeCBlockPtr  = new SrType("*cblock", CLASS_POINTER, 1, mTypeCBlock ) );
...
QString SrCompiler::macroExpansion(const QString mac, const QString def)
  {
